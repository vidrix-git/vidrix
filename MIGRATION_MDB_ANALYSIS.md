...
Uma tentativa posterior de exportar uma cópia lógica atual foi interrompida por bloqueio de conectividade TCP entre o ambiente de execução e o MySQL Azure. A regra de firewall temporária, limitada ao IP do ambiente e criada apenas para esse teste, foi removida imediatamente após a falha. O servidor Azure MySQL mantém recuperação nativa por ponto no tempo com retenção de sete dias e ciclo de backup de 24 horas. Essa proteção é complementar à auditoria e não é apresentada como uma cópia lógica pré-migração; para retenção fora desse período, uma exportação deve ser executada a partir de uma rede autorizada ou uma rotina de backup gerida pela infraestrutura Azure.
