...
export default function CounterSale() {
  ...
  return <div className="mx-auto max-w-6xl space-y-6" data-keyboard-scope>
    ...
    <Dialog open={showPriceDecision} onOpenChange={setShowPriceDecision}><DialogContent><DialogHeader><DialogTitle>Próximo passo do atendimento</DialogTitle><DialogDescription>Deseja incluir outro produto ou seguir para a escolha entre orçamento e venda?</DialogDescription></DialogHeader><DialogFooter><Button type="button" variant="outline" onClick={addItemFromPriceDecision}>Adicionar novo produto</Button><Button type="button" onClick={focusOutcomeChoice}>Finalizar atendimento</Button></DialogFooter></DialogContent></Dialog>

    <Card id="counter-sale-outcome"><CardContent className="flex flex-col gap-2 pt-6 sm:flex-row sm:justify-end"><Button ref={outcomeChoiceRef} size="lg" variant="outline" onClick={() => setOutcome("quote")} disabled={!catalogReady}><FileText className="mr-2 h-5 w-5" />Salvar como orçamento</Button><Button size="lg" onClick={() => setOutcome("sale")} disabled={!catalogReady}><ShoppingBasket className="mr-2 h-5 w-5" />Concluir como venda</Button></CardContent></Card>
