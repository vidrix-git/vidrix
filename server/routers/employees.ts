...
/** Administração de vendedores. Contas administrativas não são expostas para alteração aqui. */
export const employeesRouter = router({
  ...
  create: adminProcedure.input(createSellerSchema).mutation(async ({ input }) => {
    const created = await localRegister(input.name, input.email.toLowerCase(), input.password, "seller");
  ...
  update: adminProcedure.input(updateSellerSchema).mutation(async ({ input }) => {
    const db = await getDb();
    if (!db) throw new Error("Database not available");
    ...
    const result = await db.update(users).set(data).where(and(eq(users.id, input.id), eq(users.role, "seller")));
  ...
  delete: adminProcedure.input(z.object({ id: z.number().int().positive() })).mutation(async ({ input, ctx }) => {
    ...
    await db.delete(users).where(and(eq(users.id, input.id), eq(users.role, "seller"), ne(users.id, ctx.user.id)));
    return { success: true };
  }),
});
